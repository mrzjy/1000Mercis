...
def ttv_generation_csr(sourcePath, savePath, filename, test_size=0.15, train_size=0.7, stratify=None, random_state = 50):
    print ('************************************************')
    ...
    X = X.todense() 
    X_wait, X_test, y_wait, y_test = train_test_split( X, y, test_size=test_size, stratify=y,random_state=random_state)
    ...
    X_train, X_val, y_train, y_val = train_test_split( X_wait, y_wait, train_size=relative_train_size, stratify=y_wait,random_state=random_state)
    ...
    save_sparse_csr(savePath, filename+'_X_train', X_train)
    save_labels(savePath, filename+'_y_train', y_train)
    save_sparse_csr(savePath, filename+'_X_val', X_val)
    save_labels(savePath, filename+'_y_val', y_val)
