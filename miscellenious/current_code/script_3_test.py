...
def SMOTE_based(sourcePath):
    ...
    print y_train.shape
    X_2 = sps.vstack((X_train, X_val), format='csr')
    y = np.hstack((y_train,y_val))
    # y = concatenate((y_train,y_val), axis = 0)
    X, y_2 = sm.fit_sample(X_2.toarray(), y)
    X_2 = sps.csr_matrix(X)
    stop = time.time()
    ...
    for num_inters in K:
        ...
        grouping_features = f_script_3.feature_cluster_ttv(X_2.toarray(), y_2, weights = False, num_inters = num_inters)
...
def Weighted_based(sourcePath):
    ...
    X_test, y_test = static_load_csr(sourcePath, 'test')
    X_2 = sps.vstack((X_train, X_val), format='csr')
    y_2 = np.hstack((y_train,y_val))
    # y_2 = concatenate((y_train,y_val), axis = 0)
    ...
    for num_inters in K:
        print 'Algos for %d clusters'%(num_inters)
        grouping_features = f_script_3.feature_cluster_ttv(X_2.toarray(), y_2, weights = True, num_inters = num_inters)
        X1, X2, X3 = f_script_3.combine_ttv(X_train.toarray(), X_val.toarray(), X_test.toarray(), grouping_features)
        X_train_c = sps.csr_matrix(X1)
        X_val_c   = sps.csr_matrix(X2)
        X_test_c  = sps.csr_matrix(X3)
